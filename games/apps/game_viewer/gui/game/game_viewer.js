...
// human-vs-ai   = human Red vs AI Black
// ai-vs-human   = AI Red vs human Black
var gameMode = 'human-vs-ai';
var aiThinking = false;

function getAiSide() {
  if (gameMode === 'human-vs-ai') return 1; // Black
  if (gameMode === 'ai-vs-human') return 0; // Red
  return -1; // no AI
}

function isAiTurn() {
  return getAiSide() === engine.getSide();
}
...
function setGameMode(mode) {
  ...
  if (label) {
    label.textContent =
      mode === 'human-vs-human' ? '2 người chơi' :
      mode === 'ai-vs-human' ? 'AI (Đỏ) vs Người (Đen)' :
      'Người (Đỏ) vs AI (Đen)';
  }
...
function updateTurnIndicator() {
  ...
  var sideName = side === 0 ? 'Đỏ' : 'Đen';
  indicator.textContent = isAiTurn() ? '🤖 AI đang đi' : '♟ ' + sideName + ' đi';
  indicator.classList.toggle('ai-turn', isAiTurn());
}
...
function think() {
  ...
  var bestMove = engine.search(fixedDepth);
  if (!bestMove) {
    aiThinking = false;
    updateTurnIndicator();
    return;
  ...
  var sourceSquare = engine.getSourceSquare(bestMove);
  var targetSquare = engine.getTargetSquare(bestMove);
...
function movePiece(userSource, userTarget) {
  var moveString = engine.squareToString(userSource) +
  ...
  if (!gameResult || gameResult === '*') {
    if (isAiTurn()) setTimeout(think, 180);
  }
}
